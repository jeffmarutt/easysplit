...
import { Plus, Minus, ShoppingBag, Trash2, Users, AlertTriangle, Check, Edit2, Save, X, Loader2, Wallet, Images, Store, PenLine, ChevronDown, ChevronUp, Delete, PlusCircle, Tag, User, Percent, Receipt as ReceiptIcon, Settings2, Info, ChevronRight, Sparkles, Sliders, Camera, Hash, Image as ImageIcon } from 'lucide-react';
        ...
        <div className="relative z-10 bg-white dark:bg-slate-900 rounded-t-[2.5rem] px-6 pt-4 pb-safe max-w-lg mx-auto w-full shadow-2xl animate-slide-up border-t border-gray-100 dark:border-slate-800">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className={`text-xs font-bold mt-1 flex items-center gap-1.5 ${isSharedMode ? 'text-indigo-500' : 'text-teal-600'}`}>
                        {isSharedMode ? (
                           <><Users size={14} /> หารเฉลี่ย {currentTotalAllocated} คน (ของ {allocationItem.quantity} ชิ้น)</>
                        ) : (
            ...
            <div className="space-y-3 mb-8 max-h-[50vh] overflow-y-auto no-scrollbar py-2">
                {members.map(member => {
                    ...
                    return (
                        <div key={member.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <span className={`w-8 text-center font-black text-xl ${count > 0 ? 'text-teal-600' : 'text-slate-300'}`}>{count}</span>
                                <button 
                ...
                {!isCollapsed && (
                    <div className="p-4 space-y-4">
                        {receiptItems.length === 0 ? (
                            ...
                            const isMissingUnits = currentAssigned < item.quantity;
                            const isSharedMode = currentAssigned > item.quantity;
                            const isFullyAssigned = currentAssigned === item.quantity;
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex-1 pr-4">
                                            ...
                                            <div className="mt-1 flex items-center gap-1.5 text-slate-400 font-bold text-[11px]">
                                                ...
                                                {isSharedMode && (
                                                    <span className="text-indigo-500 flex items-center gap-1"><Users size={12}/> หารเฉลี่ย ({currentAssigned} คน)</span>
                                                )}
                                            </div>
                                        </div>
                                        <div className="text-right">
