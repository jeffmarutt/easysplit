...
import { ArrowLeft, Check, ChevronRight, UserPlus, ShoppingBag, Receipt, User, HelpCircle, FileText, Sparkles, Calculator, Sun, Moon, Camera, PenLine } from 'lucide-react';
...
export const ManualWizard: React.FC<ManualWizardProps> = ({
  ...
  const handleNext = () => {
    ...
    if (step === 3) onFinish(); else setStep(prev => prev + 1);
  };

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && onScanReceipt) {
        onScanReceipt(Array.from(e.target.files));
        setStep(3); // Jump straight to items after scan
    }
  };
  ...
  const renderStepContent = () => {
    switch (step) {
        ...
        return (
          <div key="step0" className="flex flex-col items-center text-center space-y-8 px-4 pt-10 animate-fade-in-up">
            ...
            <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">เลือกวิธีที่สะดวกที่สุดเพื่อเริ่มจดบิล</p>
            </div>
            <div className="space-y-4 w-full max-w-sm mx-auto">
                <div className="bg-indigo-600 p-5 rounded-3xl shadow-xl flex items-center gap-4 text-left transition-all hover:scale-[1.02] cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
                    ...
                    <div className="bg-white/20 p-3 rounded-2xl text-white"><Camera size={24}/></div>
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 text-left transition-all hover:scale-[1.02] cursor-pointer group" onClick={() => setStep(1)}>
                    ...
                    <div className="flex-1">
                        <div className="font-black text-slate-800 dark:text-white text-lg">เริ่มจดเองทีละขั้น</div>
      ...
      case 3:
        return (
          <div key="step3" className="flex flex-col h-full animate-fade-in-up">
             <div className="px-6 pt-2 pb-4 text-center">
    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col transition-colors relative">
      <div className="bg-white dark:bg-slate-900 px-4 py-3 flex justify-between items-center z-10 sticky top-0 border-b dark:border-slate-800 transition-colors">
         ...
         <div className="flex gap-2 items-center">
             {[0, 1, 2, 3].map(i => <div key={i} className={`h-2 rounded-full transition-all duration-500 ${i === step ? 'w-8 bg-teal-500' : i < step ? 'w-2 bg-teal-200 dark:bg-teal-900' : 'w-2 bg-slate-100 dark:bg-slate-800'}`} />)}
         </div>
         {onToggleTheme ? (
      ...
      <div className="p-4 bg-white dark:bg-slate-900 pb-safe pt-2 border-t border-gray-50 dark:border-slate-800 sticky bottom-0 z-20 transition-colors">
         <div className="max-w-lg mx-auto w-full">
            <button onClick={handleNext} className={`w-full py-4 rounded-[1.5rem] font-bold text-lg shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-white ${step === 3 ? 'bg-slate-900 dark:bg-teal-600 hover:bg-slate-900' : 'bg-gradient-to-r from-teal-500 to-teal-600 shadow-teal-200 dark:shadow-none'}`}>
                {step === 3 && <Calculator size={20} />} {step === 0 ? "ถัดไป" : step === 3 ? "สรุปยอดรวม" : "ถัดไป"} {step !== 3 && <ChevronRight size={20} />}
            </button>
         </div>
      </div>
